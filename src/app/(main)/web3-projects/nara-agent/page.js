...
export default function NaraAgentPage() {
    ...
    const handleGenerateWallet = async () => {
        setGenerating(true);
        try {
            const res = await fetch('/api/web3-projects/nara/generate', { method: 'POST' });
            const data = await res.json();
            if (data.success && data.data?.key) {
                setGeneratedWallet(data.data);
                showToast('New Neural Key generated!');
            } else {
                setGeneratedWallet(null);
                showToast(data.error || 'Failed to generate wallet');
            }
        } catch (e) {
            setGeneratedWallet(null);
            showToast('Network error');
    ...
    const saveWalletAlias = async (walletId) => {
        try {
            const res = await fetch(`/api/web3-projects/nara/wallets/${walletId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ alias: editAliasInput })
            });
            const data = await res.json();
            if (data.success) {
                setWallets(wallets.map(w => w.id === walletId ? data.wallet : w));
                setEditingWalletId(null);
                showToast('Wallet alias updated!');
            } else {
    ...
    const copyGeneratedPk = () => {
        if (generatedWallet && generatedWallet.key) {
            navigator.clipboard.writeText(JSON.stringify(generatedWallet.key));
            showToast('Generated PK copied to clipboard!');
        }
    };
        ...
        <div className="space-y-6 md:space-y-8 animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                                                        ...
                                                        {editingWalletId === wallet.id ? (
                                                            <div className="flex items-center gap-2 mb-1">
                                                                <input
                                                                    ...
                                                                    autoFocus
                                                                    onKeyDown={(e) => { if(e.key === 'Enter') saveWalletAlias(wallet.id); if(e.key === 'Escape') setEditingWalletId(null); }}
                                                                    className="bg-black border border-green-500/40 text-xs font-black text-white px-2 py-0.5 rounded focus:outline-none focus:border-green-500 w-full max-w-[200px]"
                                                                />
                                                            ...
                                                            <div className="flex items-center gap-2 mb-1 group/alias">
                                                                ...
                                                                <button 
                                                                    onClick={() => { setEditingWalletId(wallet.id); setEditAliasInput(wallet.alias || ''); }}
                                                                    className="opacity-0 group-hover/alias:opacity-100 text-green-500/40 hover:text-green-500 transition-opacity"
                                                                >
                    ...
                    <div className="bg-[#050a05] border border-green-500/20 rounded-3xl p-8 relative overflow-hidden group">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-green-500/5 blur-[100px] pointer-events-none group-hover:bg-green-500/10 transition-colors" />
                        <div className="relative z-10">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                                <div>
                                    <h2 className="text-sm font-black text-white tracking-tight uppercase">Key Generator</h2>
                                    <p className="text-[10px] text-green-500/60 font-mono tracking-widest uppercase">Create New Neural Identity</p>
                                </div>
                                <button
                                    onClick={handleGenerateWallet}
                                    disabled={generating}
                                    className="px-4 py-2 bg-black border border-green-500/30 text-green-500 hover:border-green-500 hover:bg-green-500/10 rounded-xl transition-all font-black text-[10px] uppercase tracking-[0.2em] shadow-[0_0_15px_rgba(34,197,94,0.1)] flex items-center justify-center gap-2 disabled:opacity-50 self-start sm:self-auto"
                                >
                                    <svg className={`w-3 h-3 ${generating ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>
                                    {generating ? 'GENERATING...' : 'GENERATE WALLET'}
                                </button>
                            </div>

                            {generatedWallet && (
                                <div className="p-5 bg-black border border-green-500/20 rounded-2xl animate-in fade-in zoom-in duration-300">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="bg-[#050a05] border border-green-500/10 p-3 rounded-xl max-h-24 overflow-y-auto w-full">
                                                <p className="text-xs text-green-500/60 font-mono break-all leading-relaxed whitespace-pre-wrap">
                                                    {JSON.stringify(generatedWallet.key)}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
