            ...
            {rows.map((row) => (
                <div key={row.id} className="flex items-center gap-1.5 group">
                    ...
                    <input
                        ...
                        className="flex-1 min-w-0 bg-[#1e1e2f]/50 border-2 border-[#DA4848]/30 rounded-none px-2.5 py-1.5 text-xs text-[#F7F6E5] placeholder-gray-600 focus:outline-none focus:border-[#76D2DB]/50 font-mono transition-none"
                    />
                    <input
                        value={row.val}
    ...
    return (
        <div className="fixed inset-0 z-200 flex items-center justify-center p-4">
            ...
            <div className="relative bg-[#0B0F1A] border-2 border-[#DA4848]/40 rounded-none w-full max-w-2xl max-h-[80vh] flex flex-col ring-1 ring-white/5">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        className="flex-1 bg-transparent border-2 border-[#DA4848]/30 focus:border-[#76D2DB]/60 rounded-none px-3 py-2 text-sm text-[#F7F6E5] focus:outline-none font-mono uppercase tracking-widest"
                                    />
                                    <button
                ...
                <div className="p-4 border-t-2 border-[#DA4848]/20 bg-[#0B0F1A] flex gap-3">
                    <button onClick={onClose} className="flex-1 px-4 py-3 bg-[#DA4848]/5 hover:bg-[#DA4848]/10 text-[#DA4848] rounded-none text-xs font-black uppercase tracking-widest border-2 border-[#DA4848]/20">Cancel</button>
                    <button onClick={() => { onSave(localEnvs); onClose(); }} className="flex-1 px-4 py-3 bg-linear-to-r from-[#76D2DB] to-[#F7F6E5]/80 hover:from-[#76D2DB]/90 hover:to-[#F7F6E5] text-[#DA4848] rounded-none text-xs font-black uppercase tracking-widest">Save Changes</button>
                </div>
            </div>
        </div>
    );
            ...
            <div className="relative bg-[#0B0F1A] border-2 border-[#DA4848]/40 rounded-none w-full max-w-xl flex flex-col overflow-hidden ring-1 ring-white/5">
                ...
                <div className="p-4 space-y-4">
                    ...
                    <textarea
                        ...
                        rows={6}
                        className="w-full bg-[#05060a] border-2 border-[#DA4848]/20 focus:border-[#76D2DB]/40 rounded-none px-3 py-2 text-xs text-[#F7F6E5] placeholder-gray-700 font-mono focus:outline-none resize-none transition-none"
                        spellCheck={false}
                    />
...
export default function HttpClientPage() {
    ...
    const SidebarContent = () => (
        <div className="flex flex-col h-full text-sm">
            {/* Env switcher */}
            <div className="p-3 border-b-2 border-[#DA4848]/20">
                <div className="flex items-center gap-2">
                    <select
                        value={activeEnvId || ''}
                        onChange={e => setActiveEnvId(e.target.value || null)}
                        className="flex-1 bg-transparent border-2 border-[#DA4848]/20 rounded-none px-2.5 py-1.5 text-xs text-[#F7F6E5] font-black uppercase tracking-widest focus:outline-none focus:border-[#76D2DB]/30 appearance-none truncate font-mono"
                    >
                            ...
                            <div key={folder.id} className="mb-2">
                                <div className="flex items-center gap-1 px-1 py-1.5 rounded-none hover:bg-white/5 group border-b border-white/5">
                                    ...
                                    {renamingFolder === folder.id ? (
                                        <input autoFocus defaultValue={folder.name} onBlur={e => renameFolder(folder.id, e.target.value)} onKeyDown={e => e.key === 'Enter' && renameFolder(folder.id, e.target.value)} className="flex-1 bg-transparent focus:bg-[#1e1e2f]/10 rounded-none px-2 py-0.5 text-xs text-[#F7F6E5] focus:outline-none border border-[#76D2DB]/20 transition-none" />
                                    ) : (
                            ...
                            {isLoggedIn && (
                                <>
                                    <select
                                        value={activeEnvId || ''}
                                        onChange={e => setActiveEnvId(e.target.value || null)}
                                        className="bg-transparent border-2 border-[#76D2DB]/40 rounded-none px-3 py-2 text-[10px] font-black uppercase text-[#F7F6E5] focus:outline-none focus:border-[#76D2DB]/60 appearance-none max-w-40 font-mono"
                                    >
                                        <option value="">No Environment</option>
                                        {environments.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}
                                    </select>
                    ...
                    <div className="max-w-[1700px] mx-auto w-full px-4 md:px-8 pb-12 mt-6">
                        <div className={(isLoggedIn && sidebarOpen) ? "grid md:grid-cols-[300px_1fr] gap-8 items-start" : "flex flex-col gap-8 items-start w-full"}>
                            ...
                            {isLoggedIn && sidebarOpen && (
                                <div className="hidden md:flex flex-col bg-[#0B0F1A] border-2 border-[#DA4848]/30 rounded-none overflow-hidden h-fit ring-1 ring-white/5">
                                    <SidebarContent />
                                </div>
                            )}
                            ...
                            {isLoggedIn && sidebarOpen && (
                                <div className="md:hidden fixed inset-0 z-50">
                                    ...
                                    <div className="absolute inset-y-0 left-0 w-72 bg-[#0B0F1A] border-r-2 border-[#DA4848]/30 flex flex-col">
                                        <SidebarContent />
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="min-w-0 flex flex-col gap-6 w-full">
                                {/* Request bar */}
                                <div className="bg-[#0B0F1A] border-2 border-[#DA4848]/40 rounded-none p-3 md:p-4 flex items-center gap-2 ring-1 ring-white/5 shadow-[4px_4px_0px_0px_rgba(218,72,72,0.1)]">
                                    <select
                                        value={activeRequest.method}
                                        onChange={e => updateReq({ method: e.target.value })}
                                        className={`px-3 md:px-4 py-2 md:py-3 rounded-none text-[10px] md:text-xs font-black uppercase tracking-widest border-2 cursor-pointer focus:outline-none transition-none ${METHOD_BG[activeRequest.method]}`}
                                    >
                                    ...
                                    <div className="flex-1 relative group">
                                        <input
                                            ...
                                            className="w-full bg-transparent border-2 border-[#DA4848]/20 focus:border-[#76D2DB]/50 rounded-none px-4 py-2 md:py-3 text-[11px] md:text-sm text-[#F7F6E5] placeholder-gray-700 font-mono focus:outline-none transition-none uppercase tracking-tight"
                                        />
                                    </div>
                                    <button
                                        onClick={handleSend}
                                        disabled={isSending || !activeRequest.url}
                                        className={`flex items-center gap-2 px-6 md:px-10 py-2 md:py-3 bg-[#76D2DB] hover:bg-[#F7F6E5] text-black rounded-none text-[10px] md:text-xs font-black uppercase tracking-[0.2em] disabled:opacity-30 disabled:shadow-none group overflow-hidden relative border-2 border-[#76D2DB] transition-colors`}
                                    >
                                        <span className="relative z-10 flex items-center gap-2">
                                            {isSending ? (
                                                <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                            ) : (
                                                <svg className="w-4 h-4 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                                            )}
                                            <span className="hidden md:inline">{isSending ? 'Sending...' : 'Engage uplink'}</span>
                                        </span>
                                    </button>
                                </div>

                                {/* Request tabs */}
                                <div className="bg-[#0B0F1A] border-2 border-[#DA4848]/30 rounded-none overflow-hidden ring-1 ring-white/5 shadow-[4px_4px_0px_0px_rgba(218,72,72,0.05)]">
                                    <div className="border-b-2 border-[#DA4848]/20 px-4 flex items-center gap-0.5 overflow-x-auto bg-[#0B0F1A]">
                                        {['params', 'headers', 'body'].map(tab => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    {tab}
                                                    {counts[tab] > 0 && (
                                                        <span className="bg-[#76D2DB]/20 text-[#76D2DB] text-[9px] font-black px-1.5 py-0.5 rounded-none border border-[#76D2DB]/40 font-mono">
                                                            {counts[tab]}
                                                        </span>
                                                    )}
                                                </button>
                                            ...
                                            <div className="space-y-6">
                                                ...
                                                {activeRequest.bodyType === 'json' && (
                                                    <textarea
                                                        ...
                                                        placeholder={'{\n  "key": "value"\n}'}
                                                        rows={12}
                                                        className="w-full bg-[#05060a] border-2 border-[#DA4848]/30 focus:border-[#76D2DB]/60 rounded-none px-4 py-4 text-xs text-[#F7F6E5] placeholder-gray-800 font-mono focus:outline-none resize-none transition-none"
                                                        spellCheck={false}
                                                    />
                                                )}
                                    ...
                                    {!response && !isSending && (
                                        <div className="flex flex-col items-center justify-center py-24 text-gray-700">
                                            ...
                                            <div className="mt-8 flex items-center gap-3 px-5 py-2.5 bg-[#76D2DB]/5 border-2 border-[#DA4848]/20 rounded-none">
                                                <span className="text-[9px] font-black text-[#76D2DB]/40 tracking-[0.2em] font-mono">PRESS CTRL+ENTER TO ENGAGE</span>
                                            </div>
                                        </div>
                                    )}
                                    {isSending && (
                                    ...
                                    {response && !isSending && (
                                        <div className="">
                                            <div className="border-b border-[#DA4848]/20 px-6 py-5 flex items-center gap-4 bg-white/5">
                                                {response.error ? (
                                                    <span className="flex items-center gap-3 px-5 py-2.5 rounded-none text-[10px] font-black uppercase tracking-[0.2em] bg-[#DA4848]/10 text-[#DA4848] border-2 border-[#DA4848]/40 font-mono">
                                                    ...
                                                    <>
                                                        <span className={`px-5 py-2.5 rounded-none text-[10px] font-black uppercase tracking-widest border-2 font-mono ${STATUS_BG(response.status)} ${STATUS_COLOR(response.status)}`}>
                                                ...
                                                <div className="ml-auto flex gap-2 p-1.5 bg-[#0B0F1A] rounded-none border-2 border-white/5">
                                                    {['body', 'headers'].map(t => (
                                                        <button
                                                            key={t}
